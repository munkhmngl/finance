...
var uiController = (function() {
    ...
    var formatMoney = function(number, type){
        number = '' + number;
        var tempA = number.split('').reverse().join('');
        var tempB = '', count = 1;
        ...
        var tempC = tempB.split('').reverse().join('');
        if(tempC[0] === ',') tempC = tempC.substring(1, tempC.length);
        if(type === 'inc') tempC = '+ ' + tempC;
        else tempC = '- ' + tempC;
        return tempC;
    };
    return {
        ...
        displayPersentages: function(allPercentages){
            // зарлагын NodeList-ийг олох <buttom id=''
